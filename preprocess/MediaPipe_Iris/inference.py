...
def main():
    ...
    print(f"총 {len(high_aes_keys)}개의 이미지가 AES 점수 기준 > 5.5 충족.")

    image_paths = []
    # rank는 단일 프로세스이므로 0으로 설정
    rank = 0
    # world_size는 단일 프로세스이므로 1로 설정
    world_size = 1
    # tqdm disable 조건 수정: 단일 프로세스이므로 항상 tqdm 표시
    for k in tqdm(high_aes_keys, desc="이미지 경로 수집 중", disable=False):
        rel = k + ".jpg"
    ...
    print(f"추론할 이미지 총 {len(image_paths)}개 수집 완료.")
    ...
    # for img, img_path, center, scale, rotate, face_landmarks_detection in tqdm(dataloader, total=len(dataloader), desc="이미지 추론 중"):
    for img, img_path, center, scale, rotate in tqdm(dataloader, total=len(dataloader), desc="이미지 추론 중"):
        try:
            img_path = img_path[0] # 배치 사이즈 1이므로 첫번째 요소 사용
            input_image = img[0].numpy() # 배치 사이즈 1이므로 첫번째 요소 사용, numpy 배열로 변환
            # face_landmarks_detection = face_landmarks_detection[0]  # 배치 사이즈 1이므로 첫번째 요소 사용
