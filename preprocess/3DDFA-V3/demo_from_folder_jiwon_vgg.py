...
def main(args):
    ...
    input_base_path = args.inputpath # /mnt/data2/dataset/VGGface2_None_norm_512_true_bygfpgan
    ...
    for i, image in tqdm(enumerate(images), desc="Processing images", total=len(images)):
        try :
            # im_path = os.path.join(input_base_path, image)
            im_path = image # full path
            image = os.path.basename(image) # file name only
            ...
            id = image.replace('.png','').replace('.jpg','')
