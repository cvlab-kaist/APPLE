...
def main(args):
    ...
    # model has to be moved to device before DDP wrapper
    recon_model = face_model(args)
    # recon_model = DDP(recon_model, device_ids=[local_rank])
    ...
    images = images[args.rank::args.world_size]
    
    # save_base_path = args.savepath
    
    # tqdm for progress bar on rank 0 only
    image_iterator = enumerate(images)
    # if args.rank == 0:
    image_iterator = tqdm(image_iterator, desc=f"GPU:{args.rank}", total=len(images))

    for i, image in image_iterator:
        try :
            im_path = image # full path
            image_basename = os.path.basename(image) # file name only

            id = image_basename.replace('.png','').replace('.jpg','')
            output_dir = os.path.join(os.path.dirname(im_path), '3dmm', id)
            os.makedirs(output_dir, exist_ok=True)
            save_path = os.path.join(output_dir, f'{id}_ldm68.png')
            ...
            # Use .module to access original model attributes/methods
            ...
            #     output_dir, \
            #     im_path.split('/')[-1].replace('.png','').replace('.jpg',''))
            # my_visualize.visualize_and_output(trans_params, cv2.cvtColor(np.asarray(im), cv2.COLOR_RGB2BGR), \
            #     os.path.join(args.savepath), \
            #     im_path[i].split('/')[-1].replace('.png','').replace('.jpg',''))

            my_visualize.visualize_ldm68(trans_params, cv2.cvtColor(np.asarray(im), cv2.COLOR_RGB2BGR), \
                output_dir, \
                im_path.split('/')[-1].replace('.png','').replace('.jpg',''))
            # my_visualize = visualize(results, args)
            # my_visualize.visualize_and_output(trans_params, cv2.cvtColor(np.asarray(im), cv2.COLOR_RGB2BGR), \
            ...
            if args.rank == 0:
                print(f"Bypassing image {im_path} due to error: {e}")
            continue
...
if __name__ == '__main__':
    ...
    # rank and world_size are not needed for torchrun, but we keep them for compatibility
    # they will be overwritten by environment variables
